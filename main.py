# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 17:06:43 2021
...
#def softmax(x, axis=1):
    ...
    def validation(self, val_data,n_iteration, lable_list_val):
        ...
        print('Iteration #{}: accuracy_validation = {}%, accuracy_t5_val = {}%'.format(n_iteration,accuracy_val,
    ...
    def train(self, datasets, val_data,lable_list, lable_list_val):
        ...
        if datasets.shape[1] != n_weights-1: # ???? the columns number
            raise Exception("Wrong inputs of training!")

        
        for i in range(iterations): 
            # lable_list = np.array([lable_list])
            # X_label = np.vstack((lable_list,X.transpose())).transpose()
            # np.random.shuffle(X_label)
            # lable_list = X_label[:,0].astype(int)
            # datasets = X_label[:,1:]
            print('---------------------------') 
            n_iteration = i+1
            ...
            for k in change_list:
                ...
                #self.corss_entropy_loss(target, target)
...
if __name__ == '__main__':
    ...
    acc_train, acc_val = myPerceptron.train(X*1000,val_data,lable_list, lable_list_val)
